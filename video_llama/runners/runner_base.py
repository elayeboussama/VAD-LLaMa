...
class RunnerBase:
                    ...
                    for split_name in self.valid_splits:
                        ...
                        if val_log is not None:
                            if is_main_process():
                                ...
                                auc_metrics = val_log["AUC_all"]
                                if auc_metrics > best_auc_metric and split_name == "val":
                                    best_epoch, best_auc_metric = cur_epoch, auc_metrics

                                    self._save_checkpoint(cur_epoch, is_best=True)

                                val_log.update({"best_epoch": best_epoch})
                                val_log.update({"current_epoch": cur_epoch})
                                self.log_stats(val_log, split_name)
                        # pdb.set_trace()

            else:
                log_file = os.path.join(self.config.run_cfg.output_dir, "result/epoch_best.pickle")
                if os.path.exists(log_file):
                    ...
                    for v_name in scores_dict["prd"].keys():
                        ...
                        if p_scores.shape[0] == 1:
                            ...
                            tmp_dict[v_name] = [scores[0, :, 1]]
                        else:
                            
                            tmp_dict[v_name] = [scores[:, 1]]

                    auc_all, auc_ano = self.task.evaluate_result(tmp_dict, anno_file)
                    logging.info("Test on {} ".format(self.config.model_cfg.VAD_decoder_ckpt))
                    logging.info(
                    ...
                    if len(self.valid_splits) > 0:
                        for split_name in self.valid_splits:
                            ...
                            if val_log is not None:
                                val_log.update({"current_epoch": 'best'})
                                self.log_stats(val_log, split_name)


                    else:
                        # if no validation split is provided, we just save the checkpoint at the end of each epoch.
                        raise RuntimeError("Inference dataset is not provided!")
    ...
    @torch.no_grad()
    def eval_epoch(self, split_name, cur_epoch, skip_reload=True):
        """
        ...
        results, log = self.task.evaluation(model, data_loader, self.config.datasets_cfg.vad_instruct.test_flag, cur_epoch)
        ...
        if results is not None:
            return self.task.after_evaluation(
                ...
                epoch=cur_epoch,
                dir=self.output_dir if not self.evaluate_only else self.config.run_cfg.output_dir,
                log = log
            )
        # pdb.set_trace()
        return log
    ...
    def create_loaders(
        ...
        for dataset, bsz, is_train, collate_fn in zip(
            ...
            if isinstance(dataset, list) or isinstance(dataset, tuple):
                # pdb.set_trace()
                # if len(dataset)==1 and 'eval' in dataset[0].name:
